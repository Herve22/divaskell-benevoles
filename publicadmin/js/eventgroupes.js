...
async function loadEventDetails() {
  try {
    ...
    if (evenement && evenement.nom) {
      ...
      if (titleEl) titleEl.textContent = `📅 ${evenement.nom}`;
      if (infoEl) infoEl.textContent = `Lieu : ${evenement.lieu || "non précisé"} • Statut : ${evenement.statut}`;
    }
  } catch (err) {
...
async function loadGroupes() {
  ...
  try {
    ...
    if (groupes.length > 0) {
      ...
      ul.innerHTML = "<li class='list-group-item text-center text-muted'>Aucun groupe lié</li>";
    }
  } catch (err) {
...
function openCreneauxModal(groupeId, groupeNom) {
  console.log(`[Créneaux] Ouverture pour groupe ${groupeId}: ${groupeNom}`);
  
  const modalEl = document.getElementById("modalCreneaux");
  if (!modalEl) {
    alert("⚠️ Module créneaux non chargé");
    return;
  }
...
function setDefaultDates() {
  ...
  const debutDateEl = document.getElementById("crn-debut-date");

  const finDateEl = document.getElementById("crn-fin-date");

  const debutTimeEl = document.getElementById("crn-debut-time");

  const finTimeEl = document.getElementById("crn-fin-time");

  if (!debutDateEl || !finDateEl) return;

  let debut = new Date();

  let fin = new Date(debut.getTime() + 2 * 3600 * 1000);

  if (eventData && eventData.date_debut) debut = new Date(eventData.date_debut);

  if (eventData && eventData.date_fin) fin = new Date(eventData.date_fin);
  ...
  debutDateEl.value = fmtDate(debut);

  finDateEl.value = fmtDate(fin);

  if (debutTimeEl) debutTimeEl.value = fmtTime(debut);

  if (finTimeEl) finTimeEl.value = fmtTime(fin);
...
function resetCreneauForm() {
  ...
  if (formTitle) formTitle.textContent = "➕ Ajouter un créneau";
  if (submitBtn) {
...
async function loadCreneauxForGroupe(groupeId) {
  ...
  try {
    ...
    if (creneaux.length === 0) {
      tbody.innerHTML = '<tr><td colspan="8" class="text-center text-muted">Aucun créneau</td></tr>';
      return;
    }
...
function editCreneau(creneau) {
  ...
  // Stocker l'ID du créneau en cours d'édition
  ...
  if (formTitle) formTitle.textContent = "✏️ Modifier le créneau";
  if (submitBtn) {
...
async function addCreneau(e) {
  ...
  if (!debut || !fin) {
    alert("⚠️ Veuillez renseigner les dates de début et fin");
    return;
  }
  ...
  try {
    ...
    if (data.ok) {
      ...
      alert(isEditing ? "✅ Créneau modifié avec succès" : "✅ Créneau ajouté avec succès");
    } else {
...
function openEditModal(groupeId, nom, description, responsableId) {
  ...
  if (!idEl || !nomEl || !descEl || !respEl) {
    console.error("[openEditModal] Éléments de la modale introuvables");
    return;
  }
...
async function addGroupeToEvent() {
  ...
  try {
    ...
    if (data.ok) {
      loadGroupes();
      alert("✅ Groupe ajouté à l'événement");
    } else {
      alert("❌ Erreur: " + data.error);
    }
  } catch (err) {
...
async function removeGroupe(groupeId) {
  if (!confirm("Supprimer ce groupe de l'événement ?")) return;
  try {
    ...
    if (data.ok) {
      loadGroupes();
      alert("✅ Groupe retiré de l'événement");
    } else {
      alert("❌ Erreur: " + data.error);
    }
  } catch (err) {
